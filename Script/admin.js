document.addEventListener("DOMContentLoaded", () => {
  ...
  /* ==============================
     ENROLLMENT REQUEST HANDLER (INSERTED)
  ============================== */
  ...
  if (requestsBody) {
    requestsBody.addEventListener("click", (e) => {
      if (e.target.classList.contains("approve-btn")) {
        ...
        row.querySelector(".remarks").textContent = "Enrolled successfully";

        e.target.remove(); // remove approve button
        row.querySelector(".reject-btn").textContent = "Revoke";
      }
      ...
      if (e.target.classList.contains("reject-btn")) {
        ...
        if (currentStatus === "Approved") {
          row.querySelector(".status").textContent = "Revoked";
          row.querySelector(".remarks").textContent = "Enrollment revoked";
          e.target.textContent = "Rejected";
          e.target.disabled = true;
        } else {
          row.querySelector(".status").textContent = "Rejected";
          row.querySelector(".remarks").textContent = "Enrollment denied";
          e.target.disabled = true;
        }
      }
    });
  ...
  // ===== COURSE CARDS HANDLER =====
  const courseCards = document.querySelectorAll("#courses .dashboard-card");
  const courseDetail = document.getElementById("course-detail");
  const courseTitle = courseDetail.querySelector(".course-title");
  const backBtn = courseDetail.querySelector(".back-btn");

  // Tabs to hide (offered/archived)
  const offeredTab = document.getElementById("offeredCourses");
  const archivedTab = document.getElementById("archivedCourses");

  // Open course detail on click
  courseCards.forEach(card => {
    card.addEventListener("click", (e) => {
      // Prevent event from bubbling up to parent elements
      e.stopPropagation();
      
      const name = card.querySelector(".label-text").textContent;

      // Set course title dynamically
      courseTitle.textContent = name;

      // Hide offered/archived
      offeredTab.classList.remove("active");
      offeredTab.classList.add("hidden");
  ...
  backBtn.addEventListener("click", () => {
    ...
    // Show offered tab by default
    offeredTab.classList.remove("hidden");
    offeredTab.classList.add("active");
  });

  // Remove the data-target attribute from course cards to prevent tab switching
  document.querySelectorAll("#courses .dashboard-card").forEach(card => {
    card.removeAttribute("data-target");
  });

  // ===== RESET WHEN SWITCHING TO COURSES TAB =====
  const coursesMainTabBtn = document.querySelector('[data-tab="courses"]'); 
  // 🔼 replace with your actual sidebar button selector if different

  if (coursesMainTabBtn) {
    coursesMainTabBtn.addEventListener("click", () => {
      // Always show Offered Courses when clicking sidebar
      courseDetail.classList.remove("active");
      courseDetail.classList.add("hidden");
